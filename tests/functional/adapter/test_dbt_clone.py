...
# -- Below we define base classes for tests you import the one based on
# -- if your adapter uses dbt clone or not --
class BaseClonePossible(BaseClone):
    @pytest.mark.skip(reason="Synapse does not support cloning from different state")
...
class TestCloneNotPossibleSynapse(BaseCloneNotPossible):
...
class TestClonePossibleSynapse(BaseClonePossible):
    @pytest.fixture(autouse=True)
    def clean_up(self, project):
        yield
        with project.adapter.connection_named("__test"):
